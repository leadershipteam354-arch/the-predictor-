...
export function useKalaharianRobot(
  assets: Asset[],
  portfolio: Portfolio,
  executeTrade: (asset: Asset, amount: number, type: 'buy' | 'sell') => boolean
) {
  const [settings, setSettings] = useState<BotSettings>(() => {
    const saved = localStorage.getItem('kalaharian_settings');
  ...
  useEffect(() => {
    localStorage.setItem('kalaharian_settings', JSON.stringify(settings));
  }, [settings]);
  ...
  useEffect(() => {
    if (Object.keys(lastPrices.current).length === 0 && assets.length > 0) {
      ...
      addLog('Kalaharian AI initialized. Scanning markets...', 'info');
    }
  }, [assets, addLog]);
  ...
  const toggleBot = () => {
    setSettings(prev => {
      ...
      addLog(`Kalaharian AI Trading ${newState ? 'STARTED' : 'STOPPED'}`, newState ? 'info' : 'warning');
